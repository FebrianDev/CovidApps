        ...
        client.get(url, object : AsyncHttpResponseHandler() {
            ...
            override fun onSuccess(
                ...
                try {
                    ...
                    var j = 1f
                    for (i in 8 downTo 2) {
                        val getCases = cases.getInt(DateUtils.getDate(-i)) - cases.getInt(DateUtils.getDate(-(i + 1)))
                        listCases.add(Entry(j, getCases.toFloat()))
                        val getRecovered = recoveredCase.getInt(DateUtils.getDate(-i)) - recoveredCase.getInt(DateUtils.getDate(-(i+1)))
                        listRecovered.add(Entry(j, getRecovered.toFloat()))
                        val getDeath = deathCase.getInt(DateUtils.getDate(-i)) - deathCase.getInt(DateUtils.getDate(-(i+1)))
